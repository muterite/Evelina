...
def iztereji(kurasumma) :
    print("Par precēm iztērēji :", summa)
    kurasumma = kurasumma+summa
...
while atbilde == "nē" :   # izmanto while, jo kamēr nav iegādātas visas preces, jādodas tās nopirkt
    ...
    if veikals == "Maxima" :   # ja izvēlētais veikals ir Maxima, tad ir 30% atlaide
        summa = round(sum()-(0.3*sum()),2)
        iztereji(maxima_summa)
        ...
        if top_karte == "jā" :   # ja ir Top karte, tad atlaide ir 50%
            summa = round(sum()-(0.5*sum()), 2)
        else :    # ja nav Top karte, tad atlaide ir 20%
            summa = round(sum()-(0.2*sum()), 2)
        iztereji(top_summa)
        ...
        if x == 0 :  # ja dalījuma atlikums ir 0, tad puse no precēm (katra otrā) ir par brīvu
            ...
            summa = round(sum()-((daudzums//2)*cena), 2)
        iztereji(beta_summa)
...
def printi(shop, shopsum):
    print("Veikalā", shop, "iztērēts", shopsum)

print()
print()
printi("Rimi", rimi_summa)
printi("Maxima", maxima_summa)
printi("Top", top_summa)
printi("Labais", labais_summa)
printi("Beta", beta_summa)
print("-----------------------")
